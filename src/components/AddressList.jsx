const AddressList = ({ addresses, onEdit, onEditTime }) => (
  <div className="grid grid-cols-1 gap-4">
    {addresses.map((address) => (
      <div key={address.id} className="p-4 bg-white shadow-md rounded">
        ...
        <div className="mt-2">
          ...
          {address.working_hours.map((hours) => (
            <p key={hours.id} className="text-sm">
              {`День ${hours.day_of_the_week}: ${hours.closed ? "Закрыто" : `${hours.start_time} - ${hours.end_time}`
                }`}
            </p>
          ))}
        </div>
        ...
        >
          ✏️ Редактировать адресс
        </button>
        <button
          onClick={() => onEditTime(address)}
          className="mt-4 ml-4 px-4 py-2 bg-yellow-500 text-white rounded"
        >
          ✏️ Редактировать время работы
        </button>
      </div>
    ))}
  </div>
